...
public class MH5ScreenView extends MH5View {
     ...
     * Model section map.
     */
    private Map<String, Integer> sections = null;
    ...
    /**
     * The number of workspace column.
     */
    private int workSpaceColumnNumber = 0;

    public MH5ScreenView() {
        super();
     ...
     * @param p_iSection
     */
    public void addNestedSubview(String p_oNestedSubview, ViewModelType p_oViewModelType, boolean isFirstDetail, int p_iColumn, int p_iSection) {
        if (!this.nestedSubview.contains(p_oNestedSubview)) {
            ...
            this.columns.put(p_oNestedSubview, p_iColumn);
            this.sections.put(p_oNestedSubview, p_iSection);
            if (isFirstDetail) {
...
//		}
        ...
        Element xSection = null;
        Element xPanelColumn = null;
        if (this.isWorkspace()) {
            xPanelColumn = xNestedSubviews.addElement("masterColumn");
        }
        Map<Integer, Element> l_mColumn = new HashMap<>();
        this.workSpaceColumnNumber = 0;
        for (String currNestedSubview : this.getNestedSubview()) {
            if (currNestedSubview != null) {
                int l_iColumn = this.columns.get(currNestedSubview);
                ...
                if (this.isWorkspace()) {
                    if (!isList) {
                        if (!l_mColumn.containsKey(l_iColumn)) {
                            l_mColumn.put(l_iColumn, xNestedSubviews.addElement("workSpaceColumn"));
                            this.workSpaceColumnNumber += 1;
                        }
                        xSection = l_mColumn.get(l_iColumn).addElement("section");
                        xSection.setText(currNestedSubview);
                    } else {
                        xSection = xPanelColumn.addElement("section");
                        xSection.setText(currNestedSubview);
                    }
                }
    ...
    /**
     * Get workSpaceColumnNumber.
     *
     * @return workSpaceColumnNumber
     */
    public int getWorkSpaceColumnNumber() {
        return workSpaceColumnNumber;
    }

    /**
     * Set workSpaceColumnNumber.
     *
     * @param workSpaceColumnNumber the number of column
     */
    public void setWorkSpaceColumnNumber(int workSpaceColumnNumber) {
        this.workSpaceColumnNumber = workSpaceColumnNumber;
    }
}
