...
/**
 * Traitement de l'ajout d'un attribut
 * @author smaitre
 */
public class AddAttributeChangeType extends AbstractChangeType implements ItfChangeType {

	/**
	 * Le Logger
	 */
	private static final Logger LOG = LoggerFactory.getLogger(AddAttributeChangeType.class);
	...
	public String getSig(Change p_oChange) {
		...
		r_sSig = r_sSig + p_oChange.getNewAttribute().getName(); 
		return r_sSig;
	}
	...
	public void processChange(Change p_oChange, Document p_oDoc, Document p_oDoc2) throws AdjavaException {
		...
		if (oElement!=null) {
			LOG.debug("[AddAttributeChangeType#processChange]  "+p_oChange.getNewAttribute().getName()+"="+ p_oChange.getNewAttribute().getValue());
			oElement.addAttribute(p_oChange.getNewAttribute().getName(), p_oChange.getNewAttribute().getValue());
		}
	}
