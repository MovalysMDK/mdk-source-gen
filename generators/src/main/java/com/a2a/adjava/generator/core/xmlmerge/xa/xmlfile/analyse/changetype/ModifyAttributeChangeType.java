...
/**
 * Traitement de la modification d'un attribut
 * @author smaitre
 */
public class ModifyAttributeChangeType extends AbstractChangeType implements ItfChangeType {

	/**
	 * Logger
	 */
	private static final Logger LOG = LoggerFactory.getLogger(ModifyAttributeChangeType.class);
	...
	public String getSig(Change p_oChange) {
		String r_sSig = super.getSig(p_oChange);
		r_sSig = r_sSig + p_oChange.getOldAttribute().getName(); 		
		return r_sSig;
	}
	...
	public void processChange(Change p_oChange, Document p_oDoc, Document p_oDoc2) throws AdjavaException {
		...
		if (oElement!=null) {
			LOG.debug("[ModifyAttributeChangeType#processChange]  "+p_oChange.getNewAttribute().getName()+"="+ p_oChange.getNewAttribute().getValue());
			oElement.attribute(p_oChange.getNewAttribute().getName()).setText(p_oChange.getNewAttribute().getValue());
		}
	}
