...
/**
 * Traitement de la suppression d'un attribut
 * @author smaitre
 */
public class RemoveAttributeChangeType extends AbstractChangeType implements ItfChangeType {
	
	/**
	 * Logger
	 */
	private static final Logger LOG = LoggerFactory.getLogger(RemoveAttributeChangeType.class);
	...
	public void processChange(Change p_oChange, Document p_oDoc, Document p_oDoc2) throws AdjavaException {
		...
		if (oElement!=null) {
			LOG.debug("[RemoveAttributeChangeType#processChange]  "+p_oChange.getOldNode()+" > "+p_oChange.getOldAttribute().getName());
			oElement.attribute(p_oChange.getOldAttribute().getName()).detach();
		}
