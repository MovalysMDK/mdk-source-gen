...
public class TestMergeProcessorGen3 {
	...
	public void testMergeProcessor() {
		File oFileResult = new File(ROOT_DIV+"/newmod/result3.xml");
		File oFile = new File(ROOT_DIV+"/newmod/gen3.xml");
		if (oFile.exists()) {
			oFile.delete();
		}
		...
		try {
			oMp.process(Paths.get(ROOT_DIV+"/oldgen","gen3.xml"), Paths.get(ROOT_DIV+"/newgen","gen3.xml"), Paths.get(ROOT_DIV+"/mod/gen3.xml"), Paths.get(ROOT_DIV+"/newmod/gen3.xml"));
