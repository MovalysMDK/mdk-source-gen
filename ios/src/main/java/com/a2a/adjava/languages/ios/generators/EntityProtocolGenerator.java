...
/**
 * Entity protocol generator
 * @author lmichenaud
 *
 */
public class EntityProtocolGenerator extends AbstractIncrementalGenerator<MIOSDomain<MIOSDictionnary, MIOSModeleFactory>> {

	/**
	 * Logger for this class
	 */
	private static final Logger log = LoggerFactory.getLogger(EntityProtocolGenerator.class);

	/**
	 * Xsl template for Entity protocol 
	 */
	private static final String ENTITY_PROTOCOL_XSL = "entity-protocol.xsl";
	...
	public void genere(XProject<MIOSDomain<MIOSDictionnary, MIOSModeleFactory>> p_oProject, DomainGeneratorContext p_oContext) throws Exception {
		...
		log.debug("> EntityProtocolGenerator.genere");
		...
		for (MEntityImpl oClass : oDictionnary.getAllEntities()) {
			createProtocol(oClass, p_oProject, p_oContext);
		}
		
		log.debug("< EntityProtocolGenerator.genere: {}", oChrono.stopAndDisplay());
	}
	
	/**
	 * Genere protocol of entity
	 ...
	 */
	private void createProtocol( MEntityImpl p_oEntity,
		...
		Document xEntityDoc = DocumentHelper.createDocument(xEntity);

		String sProtocolFile = FileTypeUtils.computeFilenameForIOSInterface(
			"model", p_oEntity.getName(), p_oMProject.getSourceDir());
		
		log.debug("  generate file: {}", sProtocolFile);
		this.doIncrementalTransform(ENTITY_PROTOCOL_XSL, sProtocolFile, xEntityDoc, p_oMProject, p_oContext);
	}
}
