 ...
 * 
 */
public class EnumConverterGenerator extends AbstractIncrementalGenerator<XDomain<ModelDictionary,XModeleFactory>> {
	
	/**
	 * Logger for this class
	 */
	private static final Logger log = LoggerFactory.getLogger(EnumConverterGenerator.class);
	
	private static final String docPath = "webapp/src/app/data/model/converters";
	...
	public void genere( XProject<XDomain<ModelDictionary, XModeleFactory>> p_oMProject, DomainGeneratorContext p_oGeneratorContext) 
			throws Exception {
		log.debug("> EnumConverterGenerator.genere");
...
//			if(oEnumeration.getTypeDescription().getDefaultUiType().equals("RadioEnum")){
				createEnum(oEnumeration, p_oMProject, p_oGeneratorContext);
//			}
		}

		log.debug("< EnumConverterGenerator.genere: {}", oChrono.stopAndDisplay());
	}
	...
	protected void createEnum( MEnumeration p_oEnumeration, XProject<XDomain<ModelDictionary, XModeleFactory>> p_oProject, DomainGeneratorContext p_oContext) throws Exception {
		...
		String sEnumImplFile = FileTypeUtils.computeFilenameForJS( docPath, p_oEnumeration.getName()+"Converter");
		log.debug("  generate file: {}", sEnumImplFile);
		this.doIncrementalTransform("enum-converter-impl.xsl", sEnumImplFile, xEnumeration, p_oProject, p_oContext);
	}
