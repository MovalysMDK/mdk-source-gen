...
import static com.a2a.adjava.xmi.v24md182uml.XMI24Constants.OWNEDATTRIBUTE_ASSOCIATION_ATTR;
 ...
 * 
 */
public final class XMI24ClassReader {

	/**
	 * Logger
	 */
	private static final Logger log = LoggerFactory.getLogger(XMI24ClassReader.class);

	/**
	 * Singleton
	 */
	private static XMI24ClassReader classReader = new XMI24ClassReader();

	/**
	 * Return singleton instance
	 * @return singleton instance
	 */
	protected static XMI24ClassReader getInstance() {
		return classReader;
	}
	...
	protected <C extends UmlClass> C readClass(Element p_xClass, UmlPackage p_oPackage, UmlDictionary p_oModelDictionary, Class<C> p_oClass)
		...
		// Read documentation
		r_oClass.setDocumentation(XMI24DocReader.getInstance().readDoc(p_xClass, StringUtils.join("Class:", sName)));
	...
	private void readAttributes( Element p_xClass, UmlClass p_oUmlClass, UmlDictionary p_oModelDictonnary ) throws Exception {
		...
		for (Element xOwnedAttr : (List<Element>) p_xClass.elements(XMI24Constants.OWNEDATTRIBUTE_NODE)) {
			if ( xOwnedAttr.attributeValue(OWNEDATTRIBUTE_ASSOCIATION_ATTR) == null) {
				p_oUmlClass.addAttribute(XMI24AttributeReader.getInstance().readAttribute(xOwnedAttr, p_oUmlClass, p_oModelDictonnary));
			}
		}
	}
	...
	private void readOperations( Element p_xClass, UmlClass p_oUmlClass, UmlDictionary p_oModelDictonnary ) throws Exception {
		...
		for (Element xOperation : (List<Element>) p_xClass.elements(XMI24Constants.OWNEDOPERATION_NODE)) {
			p_oUmlClass.addOperation(XMI24OperationReader.getInstance().readOperation(xOperation, p_oUmlClass,
					p_oModelDictonnary));
		}
	}
	...
	private void readAssociationEnds( Element p_xClass, UmlClass p_oUmlClass, UmlDictionary p_oModelDictonnary ) throws Exception {
		for (Element xOwnedAttr : (List<Element>) p_xClass.elements(XMI24Constants.OWNEDATTRIBUTE_NODE)) {
			if ( xOwnedAttr.attributeValue(OWNEDATTRIBUTE_ASSOCIATION_ATTR) != null) {
				p_oUmlClass.addAssociation(XMI24AssociationReader.getInstance().readAssociationEnd(xOwnedAttr, p_oModelDictonnary));
			}
		}
	}
}
