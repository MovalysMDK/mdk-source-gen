...
public class PojoExtractor extends AbstractExtractor<IDomain<IModelDictionary, IModelFactory>> {
	...
	public void extract(UmlModel p_oModele) throws Exception {
		...
		for (MEntityImpl oClass : getDomain().getDictionnary().getAllEntities()) {
			// Check that all classes have an identifier, with the exception of embedded classes
			if (oClass.getIdentifier().isEmpty() && !oClass.isEmbedded()) {
				MessageHandler.getInstance().addError("Class has no identifier: {}", oClass.getUmlName());
			}

			// Check that embedded classes does not have identifiers
			if (oClass.getIdentifier().isEmpty() && !oClass.isEmbedded()) {
				MessageHandler.getInstance().addError("Embedded class has an identifier: {}", oClass.getUmlName());
			}
		}
	...
	protected MEntityImpl convertUmlClass(UmlClass p_oClass) throws AdjavaException {
		...
		if (checkEntity(p_oClass)) {
			...
			// Boilerplate that pass the embedded flag
			r_oClass.setEmbedded(this.pojoContext.isEmbedded(p_oClass));
			if (r_oClass.isEmbedded()) {
				// Pass the embedded initial data type
				UmlDataType oInitialUmlDataType = (UmlDataType) p_oClass.getStereotypeParameters(ExpandableTypeProcessor.EMBEDDED_STEREOTYPE)[0];
				ITypeDescription oInitialTypeDescription = getDomain().getDictionnary().getTypeDescription(
						oInitialUmlDataType.getName());
				r_oClass.setEmbeddedInitialType(oInitialTypeDescription);

				// Set the parent entity classname and the attribute name in parent entity
				r_oClass.setEmbeddedParentClassName(
						(String) p_oClass.getStereotypeParameters(ExpandableTypeProcessor.EMBEDDED_STEREOTYPE)[1]);
				r_oClass.setEmbeddedAttributeNameInParent(
						(String) p_oClass.getStereotypeParameters(ExpandableTypeProcessor.EMBEDDED_STEREOTYPE)[2]);
			}
